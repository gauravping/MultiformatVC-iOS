...
public class SDJWT {
    ...
    public init(from encodedVC: String) throws {
        ...
        guard vcParts.count >= 3 else {
            throw VerifiableCredentialSpecErrors.EncodedCredentialShouldContainAtLeastTwoParts
        }
        ...
        guard let headerb64Data = headerB64.base64urlToBase64().data(using: .utf8),
            ...
            throw GeneralError.Base64DecodingError
        }
        ...
        getHashAlgorithm(self.payload)
        
        try decodeDisclosures()
        print("Init Complete")
    }
    
    private func getHashAlgorithm(_ payload: sd_jwt_payload) {
        guard let sd_alg = self.payload._sd_alg,
              let hashAlgorithm = HashAlgorithm(rawValue: sd_alg) else {
            print("Hash Algorithm claim not found. Assuming sha-256")
            self.hashAlgorithm = HashAlgorithm.SHA_256
            return
        }
        self.hashAlgorithm = hashAlgorithm
    }
    
    private func getDisclosures(_ payload: sd_jwt_payload) {
    ...
    private func decodeDisclosures() throws {
        ...
        for disclosureB64 in self.disclosuresB64 {
            try checkDisclosureDigest(disclosureB64)
            ...
            do {
                ...
                throw GeneralError.DecodingError("Supported data type for disclosure is String, [String], [String:String], Int, Bool")
            }
        }
    }
    
    internal func checkDisclosureDigest(_ disclosure: String) throws {
        if let disclosureData = disclosure.data(using: .utf8),
           let sd = self.payload._sd {
            let disclosureDigest = self.hashAlgorithm.digest(data: disclosureData)
            if sd.contains(disclosureDigest.base64URLEncodedString()) {
                print("Success: Disclosure present in selectively disclosed digests")
            } else {
                throw VerifiableCredentialSpecErrors.SelectiveDisclosuresDigestDoesNotMatch
            }
        }
    }
